...
def it_details(url):
    ...
    return headers, myhash

def crazy_spider(urls,headers,myhash,page):
...
def ctl_spider(page_start,urls, headers, myhash):
    t=0;
    for page in range(page_start, page_start + 11):
        # 这里可以控制爬多少。js前端用字符串？
        try:
            t1 = crazy_spider(urls, headers, myhash, page)
            if (t1 == 2):
                t = 2
        except:
            print("Post请求失败！")
...
if __name__ == '__main__':
    ...
    page_start = 1
    # page=page_start
    url = "https://m.ithome.com/html/419302.htm"  # 【添加要爬取的某个热点话题链接】https://m.ithome.com/html/419302.htm
    url=it_urlneat(url)
    ...
    headers,myhash=it_details(url)
    ctl_spider(page_start,urls, headers, myhash)
    it_save(commentlist)
